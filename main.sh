...
while IFS= read -r line; do
  if [[ $line =~ inet6.*\/64 ]]; then
    ...
    echo "http_port $current_port" >> "$output_file"
    echo "acl ip$((current_port - FIRST_PORT + 1)) localport $current_port" >> "$output_file"
    echo "tcp_outgoing_address $ipv6_address ip$((current_port - FIRST_PORT + 1))" >> "$output_file"
    echo "" >> "$output_file"
